% Copyright (C) 2013, ACM. All rights reserved
...
\begin{array}{c|cccc}
  ...
  4\ell \le q^{\sfrac{1}{4}} & O\tilde{_e}(\ell\log^5(q)+\ell^3) \text{~(bit)}  & O_e(\ell^2+\Mult(\ell)\log(\ell
  q)+\Mult(\ell^i)\log(\ell^i)) & O(\Mult(\ell^i) \log(\ell^i)) \\
  4\ell \le q^{\sfrac{1}{4}} & O\tilde{_e}(\ell\log^5(q))  \text{~(bit)}  + O\tilde{_e}(\ell\sqrt{q}) & O_e(\log(q) + \Mult(\ell^i) \log(\ell^i)) & O(\Mult(\ell^i) \log(\ell^i)) \vspace{-0.5cm}
\end{array}
$$
\label{table:main}
...
\begin{proof}
  ...
  using $O(\Mult(\ell^i)\log(\ell^i))$ operations using
...
\begin{remark}
  ...
  $O_e((\Mult(\ell)+\log(q))\sqrt{q}\log(q))$ operations. This is not asymptotically good
