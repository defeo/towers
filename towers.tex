...
More generally, let $k=\F_q$, $L=\F_{q^n}$ and $k\subset F\subsetneq
L$. The Weil restriction $\Res_{L/k}\mathbb{G}_m$ is an
\emph{algebraic torus}, and the norm $\Norm_{L/F}$ induces a map
$\Res_{L/k}\mathbb{G}_m\to\Res_{F/k}\mathbb{G}_m$. Define the
\emph{maximal torus} $T_n$ as the intersection of the kernels of the
maps $\Norm_{L/F}$ for all subfields $F$. Then $T_n$ has dimension
$\euler(n)$,  is isomorphic to
$\mathbb{G}_m^{\euler(n)}$ over the algebraic closure, and
\begin{equation}
  \label{eq:Tn}
  T_n(k) \isom \{\alpha\in L^\ast \;|\; \Norm_{L/F}(\alpha) = 1 
  \text{ for all $k\subset F\subsetneq L$} \}
\end{equation}
is a subgroup of cardinality $\Phi_n(q)$~\cite{rubin+silverberg03,voskresenskii98}.
...
\begin{lemma}
  ...
  maximal torus $T_2$ is isomorphic to the \emph{Pell conic}
  \begin{equation}
    \label{eq:Pell}
    C \;:\; 
    \begin{cases}
...
\begin{proof}
  ...
  element $(x,y)$ of $\F_q(\delta)$ belongs to $T_2$ if and only if
